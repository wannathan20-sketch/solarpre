...
- 在光伏预测和负荷预测之上，加入带 SOC、功率、容量和效率约束的储能削峰调度层，给出充电、放电或待机建议。
...
|   |-- storage_dispatch.py                  # 带约束的储能削峰调度
...
储能层不是机器学习模型，而是带约束的贪心削峰调度模块。它不做生产级安全约束调度，但会显式检查功率、容量、SOC 上下限、充放电效率和备用 SOC。
...
输出包括充电/放电/待机动作、储能功率、储能后净负荷、削峰贡献、下一时刻 SOC、约束信息和文字说明。
...
- 储能策略是带约束的贪心削峰建议，不是生产级优化调度或安全约束调度。
